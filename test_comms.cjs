...
const regex = /getUnreadCommunications: async \(operatorId: string\): Promise<any\[\]> => \{[\s\S]*?return validComms\.filter\(c => !readIds\.has\(c\.id\)\)\.sort\(\(a, b\) => a\.created_at\.localeCompare\(b\.created_at\)\);\n  \},/;

const replacement = `getUnreadCommunications: async (operatorId: string): Promise<any[]> => {
    if (!supabase) throw new Error('Supabase não configurado');
    const today = new Date().toISOString().split('T')[0];
    
    console.log('[DEBUG] getUnreadCommunications called with operatorId:', operatorId);
    ...
    console.log('[DEBUG] Fetched communications:', comms);
    ...
    console.log('[DEBUG] validComms after filter:', validComms);
    ...
    console.log('[DEBUG] reads found in communication_recipients:', reads);
    ...
    console.log('[DEBUG] Final unread communications:', unread);
